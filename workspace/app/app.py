...
def page_ask_my_pdf():
    ...
    with container:
        with st.form("question_form", clear_on_submit=False):
            ...
            st.number_input('1queryに置ける参考webサイト数',1,10,1,step=1,key="websearch_num")
            query = st.text_area("質問: ", key="input")
            submitted = st.form_submit_button("質問する")
        if submitted:
            st.session_state[StSession.CHAT_QUERY] = query
            with st.spinner("ChatGPTが入力中 ..."):
                answer, file_list, cost,nums_ref = chat(st.session_state[StSession.CHAT_QUERY],llm,memory,db,st.session_state.relate_num, st.session_state.websearch_num, max_token=st.session_state[StSession.MAX_TOKEN])
            st.session_state.costs.append(cost)
