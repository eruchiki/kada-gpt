...
from lib.websearch_module import *
import re
import asyncio
...
def chat(query,llm,memory,db,relate_num=3,websearch_num=3,filter=None, max_token=4000):
    related_data,score_data = documents_search(db,query,top_k=relate_num,filter=filter)
    websearch_data = websearch(query, websearch_num)
    # print('related_data', related_data)
    # print('websearch_data', websearch_data)
    
    related_data = related_data +  websearch_data
    # print('related_data_after', related_data)
